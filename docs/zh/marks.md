...
（1）反引号（\`\`）在 Markdown 写作中，常用来包裹词条以突出显示。

（2）如果反引号包裹的是一个中文文本词，那么反引号两端不必添加半角空格。

```
例句：打开 Firefox 浏览器，点击右上角的`打开应用程序菜单`按钮，选择`扩展和主题`。
```

（3）如果反引号包裹的是一个英文文本词或者一个长词条，那么反引号两端需要添加一个半角空格。

```
例句：在命令行中直接运行 `starcoin` 命令，默认会在前台启动一个节点并尝试加入主网。

例句：使用 `@starcoin/starmask-onboarding` 发布我们的 TypeScript 类型。
```

（4）如果是中英文混排的词条，则遵循英文单词两端需要添加半角空格以及中文文本词反引号包裹的两端不必添加空格的原则来排版。 
